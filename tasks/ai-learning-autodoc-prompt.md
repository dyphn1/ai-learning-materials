...
### 步驟 1.3 — 建立缺口主題的 task 檔案

對「新興主題清單」中 docs 尚未涵蓋的主題，建立 task 檔案至
...
你必須強制挑選 **1 到 2 個主題** 進行「深度探索」與「檔案寫入」。每一輪定時任務執行都必須至少產生或修改兩篇文件，絕對禁止只做盤點規劃而不執行寫入。

優先順序：
1. 現有 docs 字數少於 1500 字或無論文引用的（補深度）
2. tasks 目錄有但 docs 尚未建立的（補廣度）
3. 第一階段新發現的缺口主題（擴充）
...
2. **其次**：docs 有但從未更新（無「更新記錄」章節）
3. **再次**：tasks 有但 docs 尚未建立的主題
4. **最後**：第一階段新發現的缺口主題
5. 每次最多處理 **2 個主題**（保證品質，避免超時）
