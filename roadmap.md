...
## OpenClaw Cron 定時任務設定

> 將以下 JSON 貼入 OpenClaw Cron 管理介面，以每日自動執行 AI 文件更新 pipeline。
...
- **Schedule:** 每日 08:00 台北時間
- **入口腳本:** `scripts/agentic_pipeline.py`
- **執行紀錄:** `logs/pipeline.log`
- **每次執行摘要:** `logs/summary.log`
- **每個 agent 行動紀錄:** `logs/agents/{role}.log`
